    ...
    public void updateScoreboard(@NotNull BedWarsGameUser user) {
        user.getBukkitPlayer().ifPresent(player -> {
            ...
            Optional.ofNullable(scoreboard.getTeam("map")).ifPresent(team -> {
                if (game.getGameMap().isPresent()) {
                    team.setPrefix(user.getMessage("scoreboard_lobby_map_value", game.getGameMap().get().getName()));
                } else {
                    team.setPrefix(user.getMessage("scoreboard_lobby_map_value", user.getMessage("scoreboard_lobby_map_value_voting")));
                }
            });

            Optional.ofNullable(scoreboard.getTeam("teamSize")).ifPresent(team -> {
                team.setPrefix(user.getMessage("scoreboard_lobby_team_size_value", game.getMainConfig().getTeamCount(), game.getMainConfig().getTeamSize()));
            });
        });
    }
}
